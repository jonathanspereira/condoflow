...
public interface UserRepository extends JpaRepository<User, String> {
    ...
    @Query("SELECT COUNT(u) FROM User u WHERE u.active = true AND u.role != com.jonathanspereira.condoflow.user.entity.Role.SUPER_ADMIN AND (:condominiumId = -1L OR u.id IN (SELECT cr.user.id FROM CondominiumRole cr WHERE cr.condominium.id = :condominiumId))")
